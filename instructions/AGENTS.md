...
| `~/Repo/agent-agnostic-harness/instructions/rules/worktree-hygiene.md` | Add, retain, archive, or remove a project checkout under `~/Repo` |
...
- **A `Done when:` predicate is a reward function, and a loop will reach it by the cheapest available route.** Scope it to assert the property you actually want, not the artifact's existence. For content artifacts - resume renditions, site copy, anything published under Freddy's name - the predicate must assert provenance (this text traces to a verified source) and never merely that the artifact built or rendered. The 2026-08-11 fabricated-bullet incident is what a single unattended pass optimising against "it rendered" produces on the first try; a convergence loop produces it five times, each time with a context window that does not remember being wrong. The same reasoning caps the loop: `/queue` iterates an item at most three times per firing and five in total, and an unconverged item becomes a blocked queue entry carrying the check's actual failing output rather than being retried until something passes. `tools/converge.ps1` owns both the predicate and the cap, because a cap written only in prose is a cap the loop can talk itself past.
- **Verification output should be silent on success and verbose only on failure.** A check that prints its whole passing test suite spends the next iteration's context on nothing, and a flooded window is where an agent starts hallucinating about files it has just read. Wrap a check so success is an exit code and failure is the output, rather than reading a green run into the context and then reasoning over it.
- Chat image attachments from Faruk frequently fail to arrive. Roughly eight attempts failed in a single session on 2026-08-24/25, with the message text landing intact every time and no image reaching either the agent or the uploads folder. So when he refers to a screenshot, an image, or a picture, do not ask him to resend it and do not report that nothing was received. Read the newest image file in `C:\Users\faruk\Repo\Dispatch images` and treat that as the referenced image, unless he names a different file or path. Only if that folder is empty or unreachable should you say so, naming the exact path checked. The reasoning matters, so that this is not over-applied: it is a workaround for a broken delivery path, not permission to guess at which image is meant. If the newest file is clearly unrelated to what he described, say what was found and ask, rather than analysing the wrong image.
...
- Concurrent sessions must not share one working tree. Before starting work in a repo another session may be in, create a guarded worktree with `tools/worktree-add.ps1` and work there. A checkout is global state: a `git checkout` in one session silently rewrites every file under every other session in that clone, and neither side is told. Launch long-running services from a pinned worktree, never from the tree people edit in. Keep no more than three active folders for one project family; read `instructions/rules/worktree-hygiene.md` before creating or removing one.
