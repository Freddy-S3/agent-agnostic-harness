...
- Report to Freddy briefly. This is a standing setting, not a per-session preference, and it is written here because it decayed twice inside one working session on 2026-08-23 while it was held only in conversation.
  ...
  - Per-router contracts, which are not uniform: `/faruk` follows this rule as written. `/sleep` says almost nothing while running and still owes a condensed wake-up report at the end, because that report is the reason the mode exists. `/freddy` is the exception and reports at ordinary length - its trace, decisions, and reasoning are what Freddy invoked it to read. Each router's `SKILL.md` carries its own contract.
