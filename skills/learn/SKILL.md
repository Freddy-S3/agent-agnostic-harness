...
1. `queue/QUEUE.md` in full, including every entry's `Log:` section. This is the primary source — queue runs narrate their own process deviations, misdiagnoses, and verification gaps here, and it is committed so it survives across sessions.
2. `status/TRACKER.md` and `status/TRACKER-ARCHIVE.md` (both gitignored, local-only). `TRACKER.md` holds currently-open assumptions; `TRACKER-ARCHIVE.md` holds ones `/status-report` has resolved and archived rather than deleted. If `TRACKER-ARCHIVE.md` doesn't exist yet or either file is empty, say so plainly in the report rather than treating it as a failure — the archive only fills up once `/status-report` runs after this skill's own fix lands, so early runs of `/learn` will lean on `queue/QUEUE.md` almost entirely, and that's expected, not a shortfall.
